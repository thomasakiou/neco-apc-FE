...
const initialFormState: MandateCreate = {
    code: '',
    mandate: '',
    conraiss_range: [],
    station: '',
...
const MandateModal: React.FC<MandateModalProps> = ({ isOpen, onClose, onSubmit, initialData, allAssignments }) => {
    ...
    const handleConraissToggle = (conraiss: string) => {
        setFormData((prev) => {
            const currentRange = prev.conraiss_range || [];
            const newRange = currentRange.includes(conraiss)
                ? currentRange.filter(c => c !== conraiss)
                : [...currentRange, conraiss];
            return { ...prev, conraiss_range: newRange };
        });
    };
            ...
            <div className="bg-white/95 dark:bg-[#121b25]/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col border border-slate-200/50 dark:border-gray-800/50 transition-colors">
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-gray-800 bg-gradient-to-r from-emerald-50/50 via-white to-teal-50/50 dark:from-emerald-900/20 dark:via-[#121b25] dark:to-teal-900/20 rounded-t-2xl transition-colors">
                    <div className="flex flex-col">
                        <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-900 to-teal-800 dark:from-emerald-400 dark:to-teal-300 tracking-tight">
                            {initialData ? 'Edit Mandate' : 'Add New Mandate'}
                        </h2>
                        <p className="text-slate-500 dark:text-slate-400 text-xs font-medium uppercase tracking-widest mt-1">Mandate Configuration</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4 flex-1 overflow-y-auto">
                    ...
                    <FloatingInput
                        ...
                        onChange={handleChange}
                        placeholder="e.g., State, BECE Custodian, SSCE Custodian"
                    />
                    ...
                    <div className="border border-slate-200 dark:border-gray-700 rounded-xl p-4 bg-slate-50 dark:bg-slate-800/50 transition-colors">
                        <label className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 block">CONRAISS Range</label>
                        <div className="grid grid-cols-5 gap-2">
                            {CONRAISS_OPTIONS.map((conraiss) => (
                                <label
                                    key={conraiss}
                                ...
                                >
                                    <input
                                        type="checkbox"
                                        checked={formData.conraiss_range?.includes(conraiss) || false}
                                        onChange={() => handleConraissToggle(conraiss)}
                ...
                <div className="flex gap-3 p-6 border-t border-slate-100 dark:border-gray-800 bg-slate-50 dark:bg-slate-800/50 rounded-b-2xl transition-colors">
                    ...
                    >
                        {loading ? 'Saving...' : (initialData ? 'Save Changes' : 'Create Mandate')}
                    </button>
                </div>
            </div>
        </div>
