...
import { TypesettingAPCRecord as APCRecord, TypesettingAPCCreate as APCCreate, TypesettingAPCUpdate as APCUpdate } from '../../types/typesettingApc';
import { getAllTypesettingAPC as getAllAPC, createTypesettingAPC as createAPC, updateTypesettingAPC as updateAPC, deleteTypesettingAPC as deleteAPC, uploadTypesettingAPC as uploadAPC, appendTypesettingAPC as appendAPC, bulkDeleteTypesettingAPC as bulkDeleteAPC, getAllTypesettingAPCRecords as getAllAPCRecords, syncTypesettingAPC } from '../../services/typesettingApc';
...
const TypesettingAPCList: React.FC = () => {
    const cached = getPageCache('TypesettingAPCList');
    ...
    // Update cache
    useEffect(() => {
        setPageCache('TypesettingAPCList', {
            data: allRecords,
            allPostings,
            page,
            limit,
    ...
    const handleSync = async () => {
        ...
        try {
            // 1. Run Backend Sync
            const result = await syncTypesettingAPC();
            ...
            if (idsToDelete.length > 0) {
                ...
                cleanupMsg = ` Cleaned up ${idsToDelete.length} invalid Typesetting staff.`;
            }

            setAlertModal({
                isOpen: true,
                title: 'Sync Successful',
                message: `Successfully synchronized Typesetting data from SDL. ${result.created_count} records were processed.${cleanupMsg}`,
                type: 'success'
            });

            fetchAllRecords(true);
        } catch (error: any) {
            console.error('Error syncing Typesetting APC records:', error);
            setAlertModal({
                isOpen: true,
                title: 'Error',
                message: 'Failed to sync Typesetting APC records: ' + (error.message || 'Unknown error'),
                type: 'error'
            });
        } finally {
            setLoading(false);
    ...
    const handleExport = useCallback(async () => {
        try {
            ...
            XLSX.writeFile(wb, `Typesetting_APC_List_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    const handlePDFExport = async () => {
        try {
            ...
            doc.save(`Typesetting_APC_Report_${new Date().toISOString().split('T')[0]}.pdf`);
            ...
            <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-slate-300">
                <div className="flex flex-col gap-2">
                    <h1 className="text-2xl md:text-3xl lg:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-900 to-teal-800 dark:from-emerald-400 dark:to-teal-500 tracking-tight">
                        Typesetting's Annual Posting Calendar (APC)
                    </h1>
                    <p className="text-sm md:text-base lg:text-lg text-slate-500 dark:text-slate-400 font-medium">Manage typesetting mandate assignments and qualifications.</p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <button
                        onClick={handleSync}
                        disabled={loading}
                        className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all text-sm"
                    >
...
export default TypesettingAPCList;