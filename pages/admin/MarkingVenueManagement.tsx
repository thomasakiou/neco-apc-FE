...
const MarkingVenueManagement: React.FC = () => {
    const cached = getPageCache('MarkingVenueManagement');

    const [states, setStates] = useState<State[]>([]);
    const [loading, setLoading] = useState(!cached);
    const [page, setPage] = useState(cached?.page || 1);
    const [limit, setLimit] = useState(cached?.limit || 10);
    const [sortField, setSortField] = useState<keyof MarkingVenue | null>(cached?.sortField || null);
    ...
    const [allVenues, setAllVenues] = useState<MarkingVenue[]>(cached?.data || []);
    ...
    const fetchAllVenues = useCallback(async (force: boolean = false) => {
        ...
        setLoading(true);
        try {
            const data = await getAllMarkingVenues(false, force);
            setAllVenues(data);
        } catch (error) {
    ...
    // Update cache
    useEffect(() => {
        setPageCache('MarkingVenueManagement', {
            data: allVenues,
            page,
            limit,
            sortField,
