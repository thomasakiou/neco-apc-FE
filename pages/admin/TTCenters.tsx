...
const TTCenters: React.FC = () => {
    ...
    const fetchCenters = useCallback(async (force: boolean = false) => {
        ...
        try {
            ...
            setAlertModal({
                isOpen: true,
                title: 'Error',
                message: 'Failed to fetch Trial Testing centers.',
                type: 'error'
            });
        } finally {
            setLoading(false);
    ...
    const handleEdit = (center: TTCenter) => {
    ...
    const handleDelete = async (id: string) => {
        setAlertModal({
            ...
            message: 'Are you sure you want to delete this Trial Testing center?',
            type: 'warning',
            onConfirm: async () => {
                try {
                    await deleteTTCenter(id);
                    fetchCenters();
                    setAlertModal({
                        isOpen: true,
                        title: 'Success',
                        message: 'Trial Testing center deleted successfully.',
                        type: 'success'
                    });
                } catch (error) {
                    setAlertModal({
                        isOpen: true,
                        title: 'Error',
                        message: 'Failed to delete Trial Testing center.',
                        type: 'error'
                    });
                }
            }
    ...
    const handleSubmit = async (data: TTCenterCreate) => {
        try {
            if (editingCenter) {
                await updateTTCenter(editingCenter.id, data);
            } else {
                await createTTCenter(data);
            }
            fetchCenters();
            ...
            setAlertModal({
                isOpen: true,
                title: 'Success',
                message: `Trial Testing center ${editingCenter ? 'updated' : 'created'} successfully.`,
                type: 'success'
            });
        } catch (error) {
            setAlertModal({
                isOpen: true,
                title: 'Error',
                message: `Failed to ${editingCenter ? 'update' : 'create'} Trial Testing center.`,
                type: 'error'
            });
        }
    };
    ...
    const isAllSelected = filteredCenters.length > 0 && filteredCenters.every(c => selectedIds.has(c.id));
                ...
                <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-slate-300 dark:border-gray-800">
                    <div className="flex flex-col gap-2">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">
                            {stateFilter ? `TT centers in ${stateFilter}` : 'Manage Trial Testing examination centers'}
                        </p>
                    </div>
                    <div className="flex gap-3">
                        ...
                        <button
                            onClick={() => {
                                ...
                                link.setAttribute("download", "tt_centers_template.csv");
                                ...
                                uploadTTCenters(file)
                                    ...
                                    .catch(error => {
                                        setAlertModal({
                                            isOpen: true,
                                            title: 'Upload Failed',
                                            message: 'Failed to upload Trial Testing centers.',
                                            type: 'error'
                                        });
                                    })
                                    .finally(() => {
                        ...
                        <button
                            onClick={() => {
                                const headers = ['SCH. Number', 'SCH. Name', 'State', 'Status'];
                                ...
                                const link = document.createElement("a");
                                link.setAttribute("href", encodedUri);
                                link.setAttribute("download", "tt_centers_export.csv");
                                document.body.appendChild(link);
                                link.click();
                                document.body.removeChild(link);
                            }}
                                ...
                                onClick={() => {
                                    setAlertModal({
                                        ...
                                        onConfirm: async () => {
                                            setLoading(true);
                                            try {
                                                await Promise.all(Array.from(selectedIds).map((id: string) => deleteTTCenter(id)));
                                                setSelectedIds(new Set());
                                                fetchCenters();
                                                setAlertModal({
                                                    isOpen: true,
                            ...
                            <table className="w-full text-left text-sm text-slate-600 dark:text-slate-400">
                                <thead className="bg-slate-100/80 dark:bg-slate-800/50 text-slate-900 dark:text-slate-300 font-bold uppercase tracking-wider border-b border-slate-300 dark:border-gray-700">
                                    <tr>
                                        ...
                                        <SortableHeader field="sch_no" label="SCH. Number" sortField={sortField} sortDirection={sortDirection} onSort={handleSort} />
                                        <SortableHeader field="sch_name" label="SCH. Name" sortField={sortField} sortDirection={sortDirection} onSort={handleSort} />
                                    ...
                                    {displayedCenters.length === 0 ? (
                                        ...
                                        displayedCenters.map((center) => (
                                            <tr key={center.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                                                ...
                                                <td className="px-4 py-4 font-bold text-slate-700 dark:text-slate-200 text-sm">{center.sch_no || '-'}</td>
                                                <td className="px-4 py-4 font-medium text-slate-700 dark:text-slate-300 text-sm">{center.sch_name}</td>
...
interface TTCenterModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: TTCenterCreate) => Promise<void>;
    initialData?: TTCenter | null;
}

const TTCenterModal: React.FC<TTCenterModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
    const [formData, setFormData] = useState<TTCenterCreate>({
        sch_no: '',
        sch_name: '',
        state: '',
        active: true,
    ...
    useEffect(() => {
        if (initialData) {
            setFormData({
                sch_no: initialData.sch_no || '',
                sch_name: initialData.sch_name || '',
            ...
            <div className="bg-white dark:bg-[#121b25] rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-200 dark:border-gray-800 transition-colors">
                <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-gray-800 bg-gradient-to-r from-emerald-50/50 via-white to-teal-50/50 dark:from-emerald-900/20 dark:via-[#121b25] dark:to-teal-900/20 rounded-t-2xl transition-colors">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-900 to-teal-800 dark:from-emerald-400 dark:to-teal-300">
                        {initialData ? 'Edit TT Center' : 'Add New TT Center'}
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-white dark:bg-[#121b25] transition-colors">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">SCH. Number</label>
                            <input
                                type="text"
                                name="sch_no"
                                value={formData.sch_no || ''}
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">SCH. Name *</label>
                            <input
                                type="text"
                                name="sch_name"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl self-end h-12">
                            <input
                                type="checkbox"
                                name="active"
                                checked={formData.active}
                                onChange={handleChange}
                                className="w-4 h-4 rounded border-slate-300 dark:border-gray-600 text-emerald-600 focus:ring-emerald-500"
                            />
                    ...
                    <div className="flex gap-3 pt-4 border-t border-slate-100 dark:border-gray-800 bg-slate-50 dark:bg-slate-800/50 p-6 -m-6 mt-4 rounded-b-2xl transition-colors">
                        ...
                        >
                            {loading ? 'Saving...' : (initialData ? 'Update Center' : 'Create Center')}
                        </button>
                    </div>
                </form>
            </div>
