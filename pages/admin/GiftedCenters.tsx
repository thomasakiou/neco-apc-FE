...
const GiftedCenters: React.FC = () => {
    ...
    const fetchCenters = async () => {
        ...
        try {
            ...
            setAlertModal({
                isOpen: true,
                title: 'Error',
                message: 'Failed to fetch Gifted centers.',
                type: 'error'
            });
        } finally {
            setLoading(false);
    ...
    const handleEdit = (center: GiftedCenter) => {
    ...
    const handleDelete = async (id: string) => {
        setAlertModal({
            ...
            message: 'Are you sure you want to delete this Gifted center?',
            type: 'warning',
            onConfirm: async () => {
                try {
                    await deleteGiftedCenter(id);
                    fetchCenters();
                    setAlertModal({
                        isOpen: true,
                        title: 'Success',
                        message: 'Gifted center deleted successfully.',
                        type: 'success'
                    });
                } catch (error) {
                    setAlertModal({
                        isOpen: true,
                        title: 'Error',
                        message: 'Failed to delete Gifted center.',
                        type: 'error'
                    });
                }
            }
    ...
    const handleSubmit = async (data: GiftedCenterCreate) => {
        try {
            const paddedData = {
                ...data,
            ...
            if (editingCenter) {
                await updateGiftedCenter(editingCenter.id, paddedData);
            } else {
                await createGiftedCenter(paddedData);
            }
            fetchCenters();
            ...
            setAlertModal({
                isOpen: true,
                title: 'Success',
                message: `Gifted center ${editingCenter ? 'updated' : 'created'} successfully.`,
                type: 'success'
            });
        } catch (error) {
            setAlertModal({
                isOpen: true,
                title: 'Error',
                message: `Failed to ${editingCenter ? 'update' : 'create'} Gifted center.`,
                type: 'error'
            });
        }
    };
                ...
                <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-gray-800">
                    <div className="flex flex-col gap-2">
                        <h1 className="text-2xl md:text-3xl lg:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-900 via-teal-800 to-emerald-700 dark:from-emerald-400 dark:via-teal-300 dark:to-emerald-500 tracking-tight">
                            Gifted Centers {stateFilter && `- ${stateFilter}`}
                        </h1>
                        <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">
                            {stateFilter ? `Gifted centers in ${stateFilter}` : 'Manage Gifted examination centers'}
                        </p>
                    </div>
                    <div className="flex gap-3">
                        <button
                            onClick={() => {
                                ...
                                link.setAttribute("download", "gifted_centers_template.csv");
                                ...
                                uploadGiftedCenters(file)
                                    ...
                                    .catch(error => {
                                        setAlertModal({
                                            isOpen: true,
                                            title: 'Upload Failed',
                                            message: 'Failed to upload Gifted centers.',
                                            type: 'error'
                                        });
                                    })
                                    .finally(() => {
                        ...
                        <button
                            onClick={() => {
                                ...
                                const link = document.createElement("a");
                                link.setAttribute("href", encodedUri);
                                link.setAttribute("download", "gifted_centers_export.csv");
                                document.body.appendChild(link);
                                link.click();
                                document.body.removeChild(link);
                            }}
                                ...
                                onClick={() => {
                                    setAlertModal({
                                        ...
                                        onConfirm: async () => {
                                            setLoading(true);
                                            try {
                                                await Promise.all(Array.from(selectedIds).map((id: string) => deleteGiftedCenter(id)));
                                                setSelectedIds(new Set());
                                                fetchCenters();
                                                setAlertModal({
                                                    isOpen: true,
...
interface GiftedCenterModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: GiftedCenterCreate) => Promise<void>;
    initialData?: GiftedCenter | null;
}

const GiftedCenterModal: React.FC<GiftedCenterModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
    const [formData, setFormData] = useState<GiftedCenterCreate>({
        code: '',
        name: '',
        numb_of_cand: 0,
        state: '',
        within_capital: false,
    ...
    useEffect(() => {
        if (initialData) {
            setFormData({
                ...
                within_capital: initialData.within_capital || false,
                outside_capital: initialData.outside_capital || false,
            ...
            <div className="bg-white dark:bg-[#121b25] rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-200 dark:border-gray-800 transition-colors">
                <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-gray-800 bg-gradient-to-r from-emerald-50/50 via-white to-teal-50/50 dark:from-emerald-900/20 dark:via-[#121b25] dark:to-teal-900/20 rounded-t-2xl transition-colors">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-900 to-teal-800 dark:from-emerald-400 dark:to-teal-300">
                        {initialData ? 'Edit Gifted Center' : 'Add New Gifted Center'}
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-white dark:bg-[#121b25] transition-colors">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Center Code</label>
                            <input
                                type="text"
                                name="code"
                                value={formData.code || ''}
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Center Name *</label>
                            <input
                                type="text"
                                name="name"
                                value={formData.name}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Number of Candidates</label>
                            <input
                                type="number"
                                name="numb_of_cand"
                                value={formData.numb_of_cand}
                                onChange={handleChange}
                                min="0"
                                className="w-full h-12 px-4 rounded-xl border border-slate-200 dark:border-gray-700 bg-slate-50 dark:bg-slate-800/50 focus:bg-white dark:focus:bg-[#0b1015] focus:border-emerald-500 transition-all text-slate-700 dark:text-slate-300"
                                placeholder="0"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                            <input
                                type="checkbox"
                                name="within_capital"
                                checked={formData.within_capital || false}
                                onChange={handleChange}
                                className="w-4 h-4 rounded border-slate-300 dark:border-gray-600 text-emerald-600 focus:ring-emerald-500"
                            />
                            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Within Capital</label>
                        </div>
                        <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                            <input
                                type="checkbox"
                                name="outside_capital"
                                checked={formData.outside_capital || false}
                                onChange={handleChange}
                                className="w-4 h-4 rounded border-slate-300 dark:border-gray-600 text-emerald-600 focus:ring-emerald-500"
                            />
                            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Outside Capital</label>
                        </div>
                        <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                            <input
                                type="checkbox"
