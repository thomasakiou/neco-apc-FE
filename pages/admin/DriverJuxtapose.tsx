...
import { getAllDriverAPCRecords, updateDriverAPC, getAssignmentUsage } from '../../services/driverApc';
import { getAllStaff } from '../../services/staff';
import { getPageCache, setPageCache } from '../../services/pageCache';
import { DriverAPCRecord, DriverAPCCreate } from '../../types/driverApc';
import { Staff } from '../../types/staff';
import AlertModal from '../../components/AlertModal';

interface JuxtaposeRow extends DriverAPCRecord {
...
const DriverJuxtapose: React.FC = () => {
    const navigate = useNavigate();
    const cached = getPageCache('DriverJuxtapose');
    ...
    const [editingRecord, setEditingRecord] = useState<DriverAPCRecord | null>(null);
    ...
    const fetchData = useCallback(async (force: boolean = false) => {
        ...
        try {
            const [apcRecords, sdlStaffList] = await Promise.all([
                getAllDriverAPCRecords(true, force),
            ...
            const processed: JuxtaposeRow[] = apcRecords.map(record => {
                ...
                if (sdlStaff) {
                    ...
                    // Compare Rank
                    if (normalize(record.rank) !== normalize(sdlStaff.rank)) {
                        mismatches.push(`Rank: APC("${record.rank}") vs SDL("${sdlStaff.rank}")`);
                        hasFieldMismatch = true;
                    }
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-slate-50 dark:bg-[#101922] p-8 gap-8 overflow-y-auto transition-colors duration-200">
            <div className="flex flex-col gap-2 pb-6 border-b border-slate-300 dark:border-gray-800">
                <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-900 via-amber-800 to-orange-700 dark:from-orange-400 dark:via-amber-300 dark:to-orange-500 tracking-tight">
                    Driver Juxtapose
                </h1>
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">Compare Driver APC records with Staff SDL records.</p>
                    <button
                    ...
                    >
                        <span className="material-symbols-outlined text-orange-500 group-hover:rotate-180 transition-transform duration-500 text-lg">refresh</span>
                        Refresh Data
                    </button>
                </div>
            </div>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <button
                                                        ...
                                                        title="Edit Driver APC"
                                                    >
                                                        <span className="material-symbols-outlined text-lg">edit</span>
                                                    </button>
                                                    <button
                                                        onClick={() => navigate(`/admin/drivers/apc?f=${record.file_no}`)}
            ...
            {showEditModal && (
                <APCModal
                    ...
                    onSubmit={async (data) => {
                        try {
                            if (editingRecord) {
                                await updateDriverAPC(editingRecord.id, data);
                                setAlertModal({
                                    isOpen: true,
                                    title: 'Success',
                                    message: 'Driver APC record updated successfully.',
                                    type: 'success'
                                });
                                fetchData(true);
                            }
...
const APCModal: React.FC<{ isOpen: boolean; onClose: () => void; onSubmit: (data: DriverAPCCreate) => Promise<void>; initialData?: DriverAPCRecord | null; }> = ({ isOpen, onClose, onSubmit, initialData }) => {
    const [formData, setFormData] = useState<DriverAPCCreate>({ file_no: '', name: '', conraiss: '', station: '', active: true });
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 transition-all duration-300">
            <div className="bg-white/95 dark:bg-[#121b25]/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] border border-slate-200/50 dark:border-gray-700/50">
                <div className="flex-none flex items-center justify-between p-6 border-b border-slate-100 dark:border-gray-700 bg-gradient-to-r from-orange-50/50 via-white to-amber-50/50 dark:from-orange-900/20 dark:via-[#121b25] dark:to-amber-900/20 rounded-t-2xl">
                    <div className="flex flex-col">
                        <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-900 to-amber-800 dark:from-orange-400 dark:to-amber-400 tracking-tight">
                            {initialData ? 'Edit Driver APC Record' : 'Add Driver APC Record'}
                        </h2>
                ...
                <div className="flex-1 overflow-y-auto p-8">
                    <form id="driver-apc-juxtapose-form" onSubmit={handleSubmit} className="flex flex-col gap-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <FloatingInput label="Rank" value={formData.rank || ''} onChange={(val: string) => setFormData({ ...formData, rank: val })} />
                            <FloatingInput label="Type of Vehicle" value={formData.type_of_vehicle || ''} onChange={(val: string) => setFormData({ ...formData, type_of_vehicle: val })} />
                ...
                <div className="flex-none flex justify-end gap-4 p-6 border-t border-slate-100 dark:border-gray-700 bg-white dark:bg-[#121b25]">
                    ...
                    <button type="submit" form="driver-apc-juxtapose-form" disabled={loading} className="px-6 py-2.5 rounded-lg bg-gradient-to-r from-orange-600 to-amber-600 text-white font-bold text-sm shadow-lg hover:shadow-orange-500/20 hover:-translate-y-0.5 transition-all disabled:opacity-50">
...
const FloatingInput = ({ label, value, onChange, type = 'text' }: any) => (
    <div className="relative flex flex-col gap-1">
        ...
        <input
            type={type}
            className="w-full h-10 px-4 rounded-lg border border-slate-200 dark:border-gray-700 bg-slate-50 dark:bg-[#0b1015] focus:bg-white dark:focus:bg-[#0b1015] focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all font-bold text-sm text-slate-700 dark:text-slate-200"
            value={value}
...
export default DriverJuxtapose;
