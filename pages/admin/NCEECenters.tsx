...
const NCEECenters: React.FC = () => {
    ...
    const fetchCenters = useCallback(async (force: boolean = false) => {
        ...
        try {
            ...
            setAlertModal({
                isOpen: true,
                title: 'Error',
                message: 'Failed to fetch NCEE centers.',
                type: 'error'
            });
        } finally {
            setLoading(false);
    ...
    const handleEdit = (center: NCEECenter) => {
    ...
    const handleDelete = async (id: string) => {
        setAlertModal({
            ...
            message: 'Are you sure you want to delete this NCEE center?',
            type: 'warning',
            onConfirm: async () => {
                try {
                    await deleteNCEECenter(id);
                    fetchCenters();
                    setAlertModal({
                        isOpen: true,
                        title: 'Success',
                        message: 'NCEE center deleted successfully.',
                        type: 'success'
                    });
                } catch (error) {
                    setAlertModal({
                        isOpen: true,
                        title: 'Error',
                        message: 'Failed to delete NCEE center.',
                        type: 'error'
                    });
                }
            }
    ...
    const handleSubmit = async (data: NCEECenterCreate) => {
        try {
            ...
            setAlertModal({
                isOpen: true,
                title: 'Success',
                message: `NCEE center ${editingCenter ? 'updated' : 'created'} successfully.`,
                type: 'success'
            });
        } catch (error) {
            setAlertModal({
                isOpen: true,
                title: 'Error',
                message: `Failed to ${editingCenter ? 'update' : 'create'} NCEE center.`,
                type: 'error'
            });
        }
    };
...
interface NCEECenterModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: NCEECenterCreate) => Promise<void>;
    initialData?: NCEECenter | null;
}

const NCEECenterModal: React.FC<NCEECenterModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
    const [formData, setFormData] = useState<NCEECenterCreate>({
        code: '',
        name: '',
        numb_of_cand: 0,
        state: '',
            ...
            <div className="bg-white dark:bg-[#121b25] rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-200 dark:border-gray-800 transition-colors">
                <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-gray-800 bg-gradient-to-r from-emerald-50/50 via-white to-teal-50/50 dark:from-emerald-900/20 dark:via-[#121b25] dark:to-teal-900/20 rounded-t-2xl transition-colors">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-900 to-teal-800 dark:from-emerald-400 dark:to-teal-300">
                        {initialData ? 'Edit NCEE Center' : 'Add New NCEE Center'}
                    </h2>
                    <button
                        onClick={onClose}
