...
import { getAllTypesettingAPC, updateTypesettingAPC, assignmentFieldMap } from './typesettingApc';
import { Assignment } from '../types/assignment';
import { getAllTypesettingPostings, bulkCreateTypesettingPostings, bulkDeleteTypesettingPostings } from './typesettingPosting';
import { TypesettingPostingCreate as PostingCreate } from '../types/typesettingPosting';

export const getTypesettingAssignmentBoardData = async (assignment: Assignment): Promise<AssignmentBoardData> => {
    // 1. Fetch data
    const [mandates, allTypesettingApc, allTypesettingPostings] = await Promise.all([
        getMandatesByAssignment(assignment),
        getAllTypesettingAPC(0, 100000),
        getAllTypesettingPostings(true)
    ]);
    ...
    allTypesettingApc.items.forEach((record: any) => {
    ...
    allTypesettingPostings.forEach(p => {
    ...
    // 4. Distribute Typesetting Staff
    const unassignedStaff: StaffMandateAssignment[] = [];

    allTypesettingApc.items.forEach((typesetter: any) => {
        const normalizedStaffFileNo = typesetter.file_no.padStart(4, '0');
        ...
        const totalAllotted = typesetter.count || 0;
        ...
        const staffAssignment: StaffMandateAssignment = {
            id: typesetter.id,
            staff_no: typesetter.file_no,
            staff_name: typesetter.name,
            rank: typesetter.rank || 'TYPESETTER',
            current_station: typesetter.station || '',
            qualification: typesetter.qualification || '',
            conr: typesetter.conraiss || '',
            mandate_id: mandateId || null,
            apc_id: typesetter.id,
        ...
        if (mandateId) {
            ...
            const val = typesetter[fieldName as keyof any];
...
export const bulkSaveTypesettingAssignments = async (
    payload: {
    ...
    // Load dependencies
    const [allPostings, allTypesettingApc, mandates] = await Promise.all([
        getAllTypesettingPostings(true),
        getAllTypesettingAPC(0, 100000),
        getMandatesByAssignment(assignment)
    ]);
    const postingMap = new Map<string, any>(allPostings.map(p => [String(p.file_no).padStart(4, '0'), p]));
    const typesettingApcMap = new Map<string, any>(allTypesettingApc.items.map(a => [String(a.file_no).padStart(4, '0'), a]));
    ...
    for (const change of changes) {
        ...
        const typesettingApcRecord = typesettingApcMap.get(normalizedStaffNo);

        if (!typesettingApcRecord && change.action === 'add') throw new Error(`Typesetter ${change.staff.staff_no} not found in records.`);

        const allottedCount = numberOfNights !== undefined ? numberOfNights : (typesettingApcRecord?.count || 0);
        const apcLimit = typesettingApcRecord?.count || 0;
        ...
        if (change.action === 'add' || change.action === 'move') {
            ...
            // Sync with Typesetting APC
            if (change.action === 'add' && typesettingApcRecord && fieldName) {
                const { id, created_at, updated_at, created_by, updated_by, ...clean } = typesettingApcRecord;
                await updateTypesettingAPC(id, { ...clean, [fieldName]: '' });
            }
            ...
            if (typesettingApcRecord && fieldName) {
                const { id, created_at, updated_at, created_by, updated_by, ...clean } = typesettingApcRecord;
                await updateTypesettingAPC(id, { ...clean, [fieldName]: 'Returned' });
            }
    ...
    if (modifiedStaffNos.size > 0) {
        ...
        await bulkCreateTypesettingPostings({ items: batch });
    }
};
