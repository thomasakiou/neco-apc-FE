...
const AssignmentConfig: React.FC = () => {
    ...
    useEffect(() => {
        const fetchMandates = async () => {
            try {
                const response = await fetch('/api/mandates?limit=10000&include_inactive=true');
                if (!response.ok) throw new Error('Failed to fetch all mandates');
                const data = await response.json();
                setAllMandates(data.items || []);
            } catch (error) {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            showAlert('Upload Results', `Created: ${result.created_count}, Skipped: ${result.skipped_count}, Errors: ${result.error_count}`, 'info');
            fetchAssignments();
        } catch (error: any) {
    ...
    return (
        <div className="p-8 max-w-[1800px] mx-auto bg-slate-50 dark:bg-[#101922] transition-colors duration-200 min-h-screen">
