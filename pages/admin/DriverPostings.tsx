...
import { getAllDriverAPCRecords, assignmentFieldMap as driverApcFieldMap } from '../../services/driverApc';
import { getAllAssignments } from '../../services/assignment';
import { getAllMandates } from '../../services/mandate';
import { getAllStates } from '../../services/state';
import { bulkCreateDriverPostings, getAllDriverPostings, bulkDeleteDriverPostings } from '../../services/driverPosting';
import { DriverAPCRecord } from '../../types/driverApc';
import { Assignment } from '../../types/assignment';
import { Mandate } from '../../types/mandate';
import { DriverPostingCreate as PostingCreate, DriverPostingResponse as PostingResponse } from '../../types/driverPosting';
import { DriverFinalPostingResponse } from '../../types/driverFinalPosting';
...
import { getDriverAssignmentBoardData, bulkSaveDriverAssignments } from '../../services/driverPersonalizedPost';
...
const DriverPostings: React.FC = () => {
    ...
    // Data States
    const [allDrivers, setAllDrivers] = useState<DriverAPCRecord[]>([]);
    const [assignments, setAssignments] = useState<Assignment[]>([]);
    const [mandates, setMandates] = useState<Mandate[]>([]);
    const [existingPostings, setExistingPostings] = useState<PostingResponse[]>([]);
    const [finalPostings, setFinalPostings] = useState<DriverFinalPostingResponse[]>([]);
    ...
    const fetchInitialData = async () => {
        setLoading(true);
        try {
            const [driversData, assignmentsData, mandatesData, venuesData, postingsData, finalPostingsData, statesData, stationsData] = await Promise.all([
                getAllDriverAPCRecords(true),
                getAllAssignments(true),
                getAllMandates(true),
                getAllMarkingVenues(true),
                getAllDriverPostings(),
    ...
    const loadBoardData = async (assignmentId: string) => {
        ...
        try {
            ...
            const data = await getDriverAssignmentBoardData(assignment);
            setBoardData(data);
            setHasUnsavedChanges(false);
            setPendingChanges([]);
            setSelectedStaffIds(new Set());
    ...
    const handleBoardSaveChanges = async () => {
        ...
        try {
            ...
            await bulkSaveDriverAssignments({
    ...
    const downloadCsvTemplate = () => {
        ...
        link.setAttribute("download", "bulk_driver_assignment_template.csv");
    ...
    // Get eligible Drivers for selected assignment
    const eligibleDrivers = useMemo(() => {
        if (!selectedAssignment || allDrivers.length === 0) {
            return [];
        }
        ...
        return filtered;
    }, [selectedAssignment, allDrivers, assignments, existingPostings, finalPostings]);
    ...
    const generateRandomPostings = async () => {
        ...
        try {
            ...
            const apcField = driverApcFieldMap[assignmentCode];
            const eligibleDriversList = allDrivers.filter(d => {
                const normalizedFileNo = String(d.file_no).padStart(4, '0');
            ...
            const driverToStateMap = new Map<string, string>();

            eligibleDriversList.forEach(d => {
                const station = (d.station || '').toLowerCase();
                const matchedState = allStates.find(s => station.includes(s.name.toLowerCase()));
                if (matchedState) driverToStateMap.set(d.id, matchedState.name);
            });
            ...
            for (let stage = 0; stage <= 2; stage++) {
                ...
                while (madeProgress) {
                    ...
                    for (const vNeed of shuffledVenues) {
                        ...
                        const compatibleDrivers = eligibleDriversList.filter(d => {
                            if (usedDriverIds.has(d.id)) return false;
                            const dStation = (d.station || '').toLowerCase();
                            const dZone = stationToZoneMap.get(dStation);
                            const dState = driverToStateMap.get(d.id);

                            const isStateMatch = venueState && dState === venueState;
                            const isZoneMatch = !isStateMatch && venueZone && dZone === venueZone;
                        ...
                        if (compatibleDrivers.length > 0) {
                            const pickedDriver = shuffle(compatibleDrivers)[0] as DriverAPCRecord;
                            usedDriverIds.add(pickedDriver.id);
            ...
            else info('No drivers matched criteria or quotas already met.');
        } catch (err) {
    ...
    const handleSavePostings = async (postingsToSave: PostingCreate[]) => {
        ...
        try {
            ...
            await bulkCreateDriverPostings({ items: postingsToSave });
            success(`Successfully posted ${postingsToSave.length} Drivers!`);
        ...
        <div className="flex-1 flex flex-col min-h-full bg-slate-50 dark:bg-[#0b1015] p-4 md:p-8 font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
                ...
                {activeTab === 'random' && (
                    <div className="space-y-6">
                        ...
                        <div className="bg-white dark:bg-[#121b25] p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-800 flex items-center justify-between">
                            <div>
                                <p className="text-sm text-slate-500">Eligible Drivers:</p>
                                <p className="text-3xl font-black text-emerald-600">{eligibleDrivers.length}</p>
                            </div>
                            <button onClick={generateRandomPostings} disabled={loading || eligibleDrivers.length === 0} className="bg-emerald-600 text-white px-8 py-3 rounded-xl font-bold shadow-lg h-12 flex items-center gap-2">
            ...
            <CsvUploadModal
                ...
                onUpload={(csvData) => {
                    ...
                    if (successCount > 0) success(`Successfully assigned ${successCount} drivers.`);
                }}
            />
