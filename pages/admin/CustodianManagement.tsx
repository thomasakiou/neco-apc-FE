...
import { getCustodianList, deleteCustodian, createCustodian, updateCustodian, uploadCustodianCsv, getAllCustodians, bulkDeleteCustodians } from '../../services/custodian';
import { Custodian, CustodianCreate } from '../../types/custodian';
import { getAllStates } from '../../services/state';
import { State } from '../../types/state';
import CustodianModal from './CustodianModal';
import AlertModal from '../../components/AlertModal';

const CustodianManagement: React.FC = () => {
    const [custodianList, setCustodianList] = useState<Custodian[]>([]);
    ...
    const [sortField, setSortField] = useState<keyof Custodian | null>(null);
    ...
    const filteredCustodians = custodianList.filter(custodian => {
        const matchesState = selectedState === 'All' || custodian.state_id === selectedState;
        return matchesState;
    });

    const sortedCustodians = [...filteredCustodians].sort((a, b) => {
    ...
    const allFilteredCustodians = allCustodians.filter(custodian => {
        const matchesState = selectedState === 'All' || custodian.state_id === selectedState;
        return matchesState;
    });
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            setLoading(true);
            const response = await uploadCustodianCsv(file);
            setAlertModal({
                isOpen: true,
    ...
    const fetchData = async () => {
        ...
        try {
            if (hasActiveFilters) {
                const allData = await getAllCustodians();
                const filtered = allData.filter(custodian => {
                ...
                setCustodianList(filtered.slice(startIndex, endIndex));
                setTotal(filtered.length);
            } else {
                const response = await getCustodianList(page, limit, searchTerm);
                setCustodianList(response.items);
    ...
    const fetchAllCustodians = async () => {
        try {
            ...
            setAllCustodians(data);
        } catch (error) {
            console.error('Error fetching all custodians:', error);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        setAlertModal({
            ...
            message: 'Are you sure you want to delete this custodian point? This action cannot be undone.',
            type: 'warning',
            onConfirm: async () => {
                try {
                    ...
                    setAlertModal({
                        isOpen: true,
                        title: 'Success',
                        message: 'Custodian point deleted successfully.',
                        type: 'success'
                    });
                } catch (error) {
                    ...
                    setAlertModal({
                        isOpen: true,
                        title: 'Error',
                        message: 'Failed to delete custodian point.',
                        type: 'error'
                    });
                }
            }
    ...
    const handleModalSubmit = async (data: CustodianCreate) => {
        try {
            if (editingCustodian) {
                await updateCustodian(editingCustodian.id, data);
            } else {
                await createCustodian(data);
            }
            fetchData();
            fetchAllCustodians();
            ...
            setAlertModal({
                isOpen: true,
                title: 'Error',
                message: 'Failed to save custodian point. Please check your inputs and try again.',
                type: 'error'
            });
            throw error;
        }
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            const allIds = new Set(allFilteredCustodians.map(c => c.id));
    ...
    const handleSort = (field: keyof Custodian) => {
        if (sortField === field) {
    ...
    const isAllSelected = allFilteredCustodians.length > 0 && allFilteredCustodians.every(c => selectedIds.has(c.id));

    const downloadCsvTemplate = () => {
        const headers = ['state_code', 'name', 'code', 'address', 'schools', 'active'];
        const csvContent = "data:text/csv;charset=utf-8," + headers.join(",");
        const encodedUri = encodeURI(csvContent);
        const link = document.createElement("a");
        link.setAttribute("href", encodedUri);
        link.setAttribute("download", "custodian_upload_template.csv");
    ...
    const handleBulkDelete = () => {
        if (selectedIds.size === 0) {
            setAlertModal({
                isOpen: true,
                title: 'No Selection',
                message: 'Please select at least one custodian point to delete.',
                type: 'warning'
            });
            return;
        }
        ...
        setAlertModal({
            ...
            message: `Are you sure you want to delete ${selectedIds.size} selected custodian point(s)? This action cannot be undone.`,
            type: 'warning',
            onConfirm: async () => {
                try {
                    await bulkDeleteCustodians(Array.from(selectedIds));
                    setSelectedIds(new Set());
                    fetchData();
                    fetchAllCustodians();
                    setAlertModal({
                        isOpen: true,
                        title: 'Success',
                        message: `Successfully deleted ${selectedIds.size} custodian point(s).`,
                        type: 'success'
                    });
                } catch (error) {
                    console.error('Error deleting custodians:', error);
                    setAlertModal({
                        isOpen: true,
                        title: 'Error',
                        message: 'Failed to delete some custodian points. Please try again.',
                        type: 'error'
                    });
                }
            }
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-slate-50 dark:bg-[#101922] p-4 md:p-8 gap-4 md:gap-8 overflow-y-auto transition-colors duration-200">
            <CustodianModal
                isOpen={isModalOpen}
                onClose={() => setIsModalOpen(false)}
                onSubmit={handleModalSubmit}
                initialData={editingCustodian}
            />
            ...
            <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-gray-800">
                <div className="flex flex-col gap-2">
                    <h1 className="text-2xl md:text-3xl lg:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-900 via-teal-800 to-emerald-700 dark:from-emerald-400 dark:via-teal-300 dark:to-emerald-500 tracking-tight">
                        Custodian Point Management
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">Manage custodian points and locations</p>
                </div>
                            ...
                            <table className="w-full text-left text-sm text-slate-600 dark:text-slate-400">
                                <thead className="bg-slate-100/80 dark:bg-slate-800/80 text-slate-900 dark:text-slate-300 font-bold uppercase tracking-wider border-b border-slate-200 dark:border-gray-700">
                                    <tr>
                                        ...
                                        <SortableHeader field="name" label="Custodian Name" sortField={sortField} sortDirection={sortDirection} onSort={handleSort} />
                                        ...
                                        <SortableHeader field="schools" label="Schools" sortField={sortField} sortDirection={sortDirection} onSort={handleSort} center />
                                    ...
                                    {filteredCustodians.length === 0 ? (
                                        <tr>
                                            <td colSpan={8} className="p-10 text-center">
                                                <div className="flex flex-col items-center justify-center gap-3 text-slate-400">
                                                    ...
                                                    <p className="font-medium">No custodian points found</p>
                                        ...
                                        sortedCustodians.map((custodian) => (
                                            <CustodianRow
                                                key={custodian.id}
                                                custodian={custodian}
                                                stateName={getStateName(custodian.state_id)}
                                                onEdit={handleEdit}
                                                onDelete={handleDelete}
                                                isSelected={selectedIds.has(custodian.id)}
...
const SortableHeader = ({
    ...
    field: keyof Custodian;
    label: string;
    sortField: keyof Custodian | null;
    sortDirection: 'asc' | 'desc';
    onSort: (field: keyof Custodian) => void;
    center?: boolean;
}) => {
...
const CustodianRow: React.FC<CustodianRowProps> = ({ custodian, stateName, onEdit, onDelete, isSelected, onSelect }) => {
    return (
        <tr className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
            <td className="p-4 text-center">
                <input
                    ...
                    className="rounded border-slate-300 dark:border-gray-600 dark:bg-gray-700 text-primary focus:ring-primary w-3.5 h-3.5 cursor-pointer"
                    checked={isSelected}
                    onChange={(e) => onSelect(custodian.id, e.target.checked)}
                />
            </td>
            <td className="px-4 py-4 font-bold text-slate-700 dark:text-slate-200 text-sm">{custodian.name}</td>
            <td className="px-4 py-4 text-slate-600 dark:text-slate-300 text-sm">{custodian.code || '-'}</td>
            <td className="px-4 py-4 text-slate-600 dark:text-slate-300 text-sm">{stateName}</td>
            <td className="px-4 py-4 text-slate-600 dark:text-slate-400 text-sm truncate max-w-xs">{custodian.address || '-'}</td>
            <td className="px-4 py-4 text-center text-slate-700 dark:text-slate-300 font-medium text-sm">{custodian.schools}</td>
            <td className="px-4 py-4 text-center">
                <span className={`inline-flex px-3 py-1 rounded-full text-xs font-bold ${custodian.active
