...
const BECECustodians: React.FC = () => {
    const cached = getPageCache('BECECustodians');

    const [searchParams] = useSearchParams();
    const stateFilter = searchParams.get('state');
    const [custodians, setCustodians] = useState<BECECustodian[]>(cached?.data || []);
    const [loading, setLoading] = useState(!cached);
    const [page, setPage] = useState(cached?.page || 1);
    const [limit, setLimit] = useState(cached?.limit || 10);
    const [sortField, setSortField] = useState<keyof BECECustodian | null>(cached?.sortField || null);
    ...
    const [editingCustodian, setEditingCustodian] = useState<BECECustodian | null>(null);
    ...
    const fetchCustodians = useCallback(async (force: boolean = false) => {
        ...
        try {
            if (stateFilter) {
                const data = await getBECECustodiansByState(stateFilter);
                setCustodians(data);
            } else {
                const items = await getAllBECECustodians(false, force);
                setCustodians(items);
            }
        } catch (error) {
    ...
    const handleSort = (field: keyof BECECustodian) => {
        if (sortField === field) {
    ...
    const handleExport = () => {
        ...
        link.setAttribute("download", "bece_custodians_export.csv");
    ...
    const handleTemplate = () => {
        ...
        const link = document.createElement("a");
        link.setAttribute("href", encodedUri);
        link.setAttribute("download", "bece_custodians_template.csv");
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
            ...
            <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-gray-800">
                <div className="flex flex-col gap-2">
                    <h1 className="text-2xl md:text-3xl lg:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-900 via-teal-800 to-emerald-700 dark:from-emerald-400 dark:via-teal-300 dark:to-emerald-500 tracking-tight">
                        BECE Custodians {stateFilter && `- ${stateFilter}`}
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">
                        {stateFilter ? `BECE custodian points in ${stateFilter}` : 'Manage Basic Education Certificate Examination custodian points'}
                    </p>
                </div>
...
const BECECustodianModal: React.FC<{ custodian?: BECECustodian | null; onClose: () => void; onSuccess: () => void }> = ({ custodian, onClose, onSuccess }) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4">
            <div className="bg-white dark:bg-[#121b25] rounded-2xl p-6 w-full max-w-md">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200">{custodian ? 'Edit Custodian' : 'Add Custodian'}</h2>
