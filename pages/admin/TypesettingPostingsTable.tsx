...
import { getAllTypesettingPostings, bulkDeleteTypesettingPostings, updateTypesettingPosting, deleteTypesettingPosting, bulkCreateTypesettingPostings } from '../../services/typesettingPosting';
import { getAllTypesettingAPC, updateTypesettingAPC, getAllTypesettingAPCRecords, assignmentFieldMap } from '../../services/typesettingApc';
import { getAllAssignments } from '../../services/assignment';
import { archiveTypesettingFinalPostings } from '../../services/typesettingFinalPosting';
import { TypesettingPostingResponse, TypesettingPostingCreate } from '../../types/typesettingPosting';
...
interface ReportField {
    id: string;
    label: string;
    accessor: (row: TypesettingPostingResponse) => any;
    default: boolean;
    pdfWidth?: number;
}
...
const TypesettingPostingsTable: React.FC = () => {
    const { success, error, warning } = useNotification();

    const [postings, setPostings] = useState<TypesettingPostingResponse[]>([]);
    ...
    const [swapSource, setSwapSource] = useState<TypesettingPostingResponse | null>(null);
    const [replacementSource, setReplacementSource] = useState<TypesettingPostingResponse | null>(null);
    const [isReplacementModalOpen, setIsReplacementModalOpen] = useState(false);
    const [replacementPool, setReplacementPool] = useState<TypesettingAPCRecord[]>([]);
    ...
    const fetchData = useCallback(async (force: boolean = false) => {
        try {
            ...
            const [postingsData, assignmentsData] = await Promise.all([
                getAllTypesettingPostings(force),
            ...
            error('Failed to load Typesetting postings.');
        } finally {
            setLoading(false);
        }
    }, [error]);
    ...
    const handleExecuteSwap = useCallback(async (target: TypesettingPostingResponse) => {
    ...
    const handleExecuteReplacement = useCallback(async (targetAPC: TypesettingAPCRecord) => {
        if (!replacementSource) return;
        try {
            setLoading(true);
            const { items: allApc } = await getAllTypesettingAPC(0, 100000, replacementSource.file_no);
            const sourceAPC = allApc.find(a => a.file_no === replacementSource.file_no);
            if (!sourceAPC) throw new Error("Original staff not found in Typesetting APC database.");

            const newTargetRecord: TypesettingPostingCreate = {
            ...
            await updateTypesettingAPC(sourceAPC.id, cleanSource);
            ...
            await updateTypesettingAPC(targetAPC.id, cleanTarget);

            await Promise.all([
                deleteTypesettingPosting(replacementSource.id),
                bulkCreateTypesettingPostings({ items: [newTargetRecord] })
            ]);
    ...
    const handleSingleDelete = useCallback(async (record: TypesettingPostingResponse) => {
        if (!window.confirm(`Are you sure you want to delete the posting for ${record.name}?`)) return;
        try {
            setLoading(true);
            const { items: allApc } = await getAllTypesettingAPC(0, 100000, record.file_no);
            ...
            if (apcRecord && record.assignments && record.assignments.length > 0) {
                ...
                if (hasChanges) {
                    ...
                    await updateTypesettingAPC(apcRecord.id, clean);
                }
            }
            await deleteTypesettingPosting(record.id);
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            setLoading(true);
            await bulkDeleteTypesettingPostings(Array.from(selectedIds));
    ...
    const handleCommitToFinal = async () => {
        ...
        if (!window.confirm('Are you sure you want to commit these postings to the Final Typesetting Postings?')) return;
        try {
            setLoading(true);
            await archiveTypesettingFinalPostings();
    ...
    const handleCSVExport = () => {
        ...
        link.download = `Typesetting_Postings_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    ...
    const handleExcelExport = () => {
        ...
        XLSX.writeFile(wb, `Typesetting_Postings_${new Date().toISOString().split('T')[0]}.xlsx`);
        ...
        <div className="flex-1 flex flex-col min-h-full bg-slate-50 dark:bg-[#0b1015] p-4 md:p-8 font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
            ...
            <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="mt-1 text-slate-500 dark:text-slate-400 font-medium">
                        View and manage all generated Typesetting postings.
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => fetchData(true)} disabled={loading} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white dark:bg-[#121b25] border border-slate-200 dark:border-gray-700 text-purple-600 dark:text-purple-400 font-bold text-xs shadow-sm hover:bg-purple-50 dark:hover:bg-purple-900/20">
                    ...
                    <button onClick={handleCSVExport} disabled={loading || filteredPostings.length === 0} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white dark:bg-[#121b25] border border-slate-200 dark:border-gray-700 text-purple-600 dark:text-purple-400 font-bold text-xs shadow-sm hover:bg-slate-50 dark:hover:bg-gray-800 transition-all disabled:opacity-50">
                        <span className="material-symbols-outlined text-lg">csv</span> CSV
                    </button>
                    <button onClick={handleExcelExport} disabled={loading || filteredPostings.length === 0} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs shadow-lg shadow-purple-500/20 transition-all disabled:opacity-50">
            ...
            <div className="flex items-center gap-4 mb-4">
                <span className="text-sm font-bold text-slate-500">
                    Showing <span className="text-purple-600">{paginatedPostings.length}</span> of <span className="text-purple-600">{total}</span> postings
                </span>
            </div>
                ...
                <div className="overflow-x-auto flex-1">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-300 dark:divide-gray-800">
                            {loading ? (
                                <tr><td colSpan={10} className="p-16 text-center"><div className="flex flex-col items-center gap-2"><span className="material-symbols-outlined text-4xl text-purple-500 animate-spin">progress_activity</span><span className="text-slate-500">Loading postings...</span></div></td></tr>
                                ...
                                paginatedPostings.map(p => (
                                    <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex flex-wrap gap-1">{p.assignments?.map((a: any, idx: number) => (<span key={idx} className="inline-flex px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 uppercase">{typeof a === 'string' ? a : a.name || a.code}</span>))}</div>
                                        </td>
                                        <td className="px-4 py-3 text-sm font-bold text-fuchsia-600 dark:text-fuchsia-400">{p.assignment_venue?.join(', ') || '-'}</td>
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <div className="flex items-center justify-center gap-1">
                                                <button onClick={() => { if (swapSource?.id === p.id) setSwapSource(null); else if (swapSource) handleExecuteSwap(p); else { setSwapSource(p); success(`Select another staff member to swap venues with ${p.name}`); } }} className={`p-2 rounded-lg transition-all duration-300 ${swapSource?.id === p.id ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' : swapSource ? 'text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20' : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`} title={swapSource?.id === p.id ? "Cancel Swap" : swapSource ? "Swap Venue with this Staff" : "Swap Staff Venue"}>
                                                ...
                                                <button onClick={async () => { setReplacementSource(p); setLoading(true); try { const allStaff = await getAllTypesettingAPCRecords(true); const postedCounts = new Map<string, number>(); postings.forEach(post => { const count = (post.assignments || []).length; postedCounts.set(post.file_no, (postedCounts.get(post.file_no) || 0) + count); }); const eligible = allStaff.filter(s => { if (s.file_no === p.file_no) return false; const currentPosted = postedCounts.get(s.file_no) || 0; const max = s.count || 0; const needed = p.assignments.length; return (currentPosted + needed) <= max; }); setReplacementPool(eligible); setIsReplacementModalOpen(true); } catch (e) { error("Failed to load eligible replacement staff."); } finally { setLoading(false); } }} className="p-2 text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors" title="Replace Staff">
                                ...
                                <tbody className="divide-y">
                                    {filteredReplacementPool.length === 0 ? <tr><td colSpan={5} className="p-8 text-center text-slate-500">No eligible staff found.</td></tr> : filteredReplacementPool.map(s => (
                                        <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800">
                                            ...
                                            <td className="p-3"><button onClick={() => handleExecuteReplacement(s)} className="px-3 py-1.5 bg-purple-600 text-white rounded-lg text-xs font-bold hover:bg-purple-700">Select</button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
...
export default TypesettingPostingsTable;
