...
import { getAllTypesettingPostings, bulkDeleteTypesettingPostings, deleteTypesettingPosting, updateTypesettingPosting, bulkCreateTypesettingPostings } from '../../services/typesettingPosting';
...
import { getAllTypesettingAPCRecords, updateTypesettingAPC, assignmentFieldMap, getAssignmentLimit } from '../../services/typesettingApc';
...
const TypesettingPostingReports: React.FC = () => {
    ...
    const handleExecuteReplacement = useCallback(async (targetAPC: TypesettingAPCRecord) => {
        ...
        try {
            ...
            const allAPC = await getAllTypesettingAPCRecords(false);
            ...
            await updateTypesettingAPC(sourceAPC.id, sourceUpdatePayload);
            ...
            await updateTypesettingAPC(targetAPC.id, targetUpdatePayload);
    ...
    const handleSingleDelete = async (record: TypesettingPostingResponse) => {
    ...
    const handleCommitToFinal = async () => {
        ...
        if (!window.confirm('Commit postings to Final Typesetting Postings?')) return;
        try {
            setLoading(true);
    ...
    const handleCSVExport = () => {
        ...
        try {
            ...
            link.download = `Typesetting_Posting_Report_${new Date().toISOString().split('T')[0]}.csv`;
            link.click();
    ...
    const handleExcelExport = () => {
        ...
        try {
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Typesetting Postings");
            XLSX.writeFile(wb, `Typesetting_Posting_Report_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    const handlePDFExport = async () => {
        ...
        try {
            ...
            doc.save(`Typesetting_Posting_Report_${reportTemplate}_${new Date().toISOString().split('T')[0]}.pdf`);
                                        ...
                                        <div className="flex items-center justify-center gap-1">
                                            ...
                                            <button
                                                onClick={async () => {
                                                    ...
                                                    setLoading(true);
                                                    try {
                                                        // Filter logic for replacement pool: simple exclusion of self and basic capacity check if possible
                                                        // For now, fetching all active APC records
                                                        const allAPC = await getAllTypesettingAPCRecords(true);

                                                        // Map current postings to check capacity if needed, or simply list all available staff
                                                        // Ideally we should filter out those who are already posted to max capacity.
                                                        // Since we don't have easy access to 'alloted' vs 'posted' without heavy computation,
                                                        // We will list all active staff EXCEPT the current one.
...
export default TypesettingPostingReports;
