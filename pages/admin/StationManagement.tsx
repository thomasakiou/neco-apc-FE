...
const StationManagement: React.FC = () => {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            setLoading(true);
            const response = await uploadStationCsv(file);
            setAlertModal({
                isOpen: true,
    ...
    const handleModalSubmit = async (data: StationCreate) => {
        try {
            if (editingStation) {
            ...
            setAlertModal({
                isOpen: true,
                title: 'Error',
                message: 'Failed to save station. Please check your inputs and try again.',
                type: 'error'
            });
            throw error;
        }
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            const allIds = new Set(allFilteredStations.map(s => s.id));
    ...
    const downloadCsvTemplate = () => {
        ...
        link.setAttribute("download", "station_upload_template.csv");
    ...
    const handleBulkDelete = () => {
        if (selectedIds.size === 0) {
            setAlertModal({
                isOpen: true,
                title: 'No Selection',
                message: 'Please select at least one station to delete.',
                type: 'warning'
            });
            return;
        }
        ...
        setAlertModal({
            ...
            message: `Are you sure you want to delete ${selectedIds.size} selected station(s)? This action cannot be undone.`,
            type: 'warning',
            onConfirm: async () => {
                try {
                    ...
                    setAlertModal({
                        isOpen: true,
                        title: 'Success',
                        message: `Successfully deleted ${selectedIds.size} station(s).`,
                        type: 'success'
                    });
                } catch (error) {
                    ...
                    setAlertModal({
                        isOpen: true,
                        title: 'Error',
                        message: 'Failed to delete some stations. Please try again.',
                        type: 'error'
                    });
                }
            }
                                    ...
                                    {paginatedStations.length === 0 ? (
                                        ...
                                        paginatedStations.map((station) => (
                                            <StationRow
                                                ...
                                                isSelected={selectedIds.has(station.id)}
...
const SortableHeader = ({
    ...
    field: keyof Station;
    label: string;
    sortField: keyof Station | null;
    sortDirection: 'asc' | 'desc';
    onSort: (field: keyof Station) => void;
    center?: boolean;
}) => {
