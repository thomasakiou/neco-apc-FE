...
import { getBECEMarkingVenueList, deleteBECEMarkingVenue, createBECEMarkingVenue, updateBECEMarkingVenue, uploadBECEMarkingVenueCsv, getAllBECEMarkingVenues } from '../../services/markingVenue';
import { BECEMarkingVenue, BECEMarkingVenueCreate } from '../../types/markingVenue';
...
const BECEMarkingVenues: React.FC = () => {
    const cached = getPageCache('BECEMarkingVenues');

    const [states, setStates] = useState<State[]>([]);
    const [loading, setLoading] = useState(!cached);
    const [page, setPage] = useState(cached?.page || 1);
    const [limit, setLimit] = useState(cached?.limit || 10);
    const [sortField, setSortField] = useState<keyof BECEMarkingVenue | null>(cached?.sortField || null);
    ...
    const [allVenues, setAllVenues] = useState<BECEMarkingVenue[]>(cached?.data || []);
    ...
    const [editingVenue, setEditingVenue] = useState<BECEMarkingVenue | null>(null);
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            setLoading(true);
            const response = await uploadBECEMarkingVenueCsv(file);
            setAlertModal({
                isOpen: true,
    ...
    const fetchAllVenues = useCallback(async (force: boolean = false) => {
        ...
        setLoading(true);
        try {
            const data = await getAllBECEMarkingVenues(false, force);
            setAllVenues(data);
        } catch (error) {
            console.error('Error fetching all BECE marking venues:', error);
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    // Update cache
    useEffect(() => {
        setPageCache('BECEMarkingVenues', {
            data: allVenues,
            page,
            limit,
            sortField,
    ...
    const handleDelete = async (id: string) => {
        setAlertModal({
            ...
            onConfirm: async () => {
                try {
                    await deleteBECEMarkingVenue(id);
                    fetchAllVenues();
                    setAlertModal({
                        isOpen: true,
                        title: 'Success',
    ...
    const handleEdit = (venue: BECEMarkingVenue) => {
    ...
    const handleModalSubmit = async (data: BECEMarkingVenueCreate) => {
        try {
            if (editingVenue) {
                await updateBECEMarkingVenue(editingVenue.id, data);
            } else {
                await createBECEMarkingVenue(data);
            }
            fetchAllVenues();
    ...
    const handleSort = (field: keyof BECEMarkingVenue) => {
        if (sortField === field) {
    ...
    const downloadCsvTemplate = () => {
        ...
        link.setAttribute("download", "bece_marking_venue_template.csv");
            ...
            <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-gray-800">
                <div className="flex flex-col gap-2">
                    <h1 className="text-2xl md:text-3xl lg:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-900 via-teal-800 to-emerald-700 dark:from-emerald-400 dark:via-teal-300 dark:to-emerald-500 tracking-tight">
                        BECE Marking Venue Management
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">Manage Basic Education Certificate Examination marking venues and locations</p>
                </div>
...
export default BECEMarkingVenues;
