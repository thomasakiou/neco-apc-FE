...
const SSCECustodians: React.FC = () => {
    const cached = getPageCache('SSCECustodians');

    const [searchParams] = useSearchParams();
    const stateFilter = searchParams.get('state');
    const [custodians, setCustodians] = useState<SSCECustodian[]>(cached?.data || []);
    const [loading, setLoading] = useState(!cached);
    const [page, setPage] = useState(cached?.page || 1);
    const [limit, setLimit] = useState(cached?.limit || 10);
    const [sortField, setSortField] = useState<keyof SSCECustodian | null>(cached?.sortField || null);
    ...
    const fetchCustodians = useCallback(async (force: boolean = false) => {
        ...
        try {
            if (stateFilter) {
                ...
                const items = await getAllSSCECustodians(false, force);
                setCustodians(items);
            }
        } catch (error) {
